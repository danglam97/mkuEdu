...
return new class extends Migration
{
    ...
    {
        Schema::create('international_posts', function (Blueprint $table) {
            $table->id();
            $table->string('name', 500)->comment('Tên tin tức quốc tế');
            $table->text('description')->nullable()->comment('Nội dung mô tả ngắn');
            $table->string('image', 550)->nullable()->comment('Hình ảnh đại diện');
        
            $table->integer('id_category')->comment('ID danh mục tin (FK)')->nullable();
            $table->string('slug_category')->comment('slug danh mục tin (FK)')->nullable();
    ...
    {
        Schema::dropIfExists('international_posts');
    }
};
